...
impl RpcClient {
    ...
    async fn call(&self, chain: &Chain, method: &str, params: Value) -> anyhow::Result<Value> {
        ...
        Err(last_err.unwrap_or_else(|| anyhow::anyhow!("no RPC endpoints for {}", chain.id)))
    }
    ...
    pub async fn erc20_balance(&self, chain: &Chain, token: &str, address: &str) -> anyhow::Result<u128> {
        let data = format!("0x70a08231000000000000000000000000{}", address.trim_start_matches("0x"));
        let v = self.call(chain, "eth_call", json!([{ "to": token, "data": data }, "latest"])).await?;
        hex_u128(v)
    }
    ...
    pub async fn nonce(&self, chain: &Chain, address: &str) -> anyhow::Result<u64> {
        let v = self.call(chain, "eth_getTransactionCount", json!([address, "pending"])).await?;
    ...
    pub async fn priority_fee(&self, chain: &Chain) -> u128 {
        match self.call(chain, "eth_maxPriorityFeePerGas", json!([])).await {
    ...
    pub async fn send_raw_transaction(&self, chain: &Chain, raw: &str) -> anyhow::Result<String> {
        ...
        v.as_str().map(|s| s.to_string()).ok_or_else(|| anyhow::anyhow!("sendRawTransaction returned non-string"))
    }
    ...
    pub async fn wait_for_receipt(
        ...
        loop {
            if let Ok(v) = self.call(chain, "eth_getTransactionReceipt", json!([tx_hash])).await {
                if v.is_object() {
                    return Ok(v);
                }
            }
    ...
    pub async fn erc20_decimals(&self, chain: &Chain, token: &str) -> u8 {
        let data = "0x313ce567";
        match self.call(chain, "eth_call", json!([{ "to": token, "data": data }, "latest"])).await {
    ...
    pub fn resolve_token(&self, _chain: &Chain, token: &str) -> Option<(String, bool)> {
        ...
        if t.is_empty() || t == "native" || t == "eth" || t == "bnb" || t == "matic" || t == "pol" || t == "avax" || t == "ftm" {
...
mod tests {
    ...
    fn hex_parsing() {
        ...
        assert_eq!(hex_u128(json!("0xde0b6b3a7640000")).unwrap(), 1_000_000_000_000_000_000);
    ...
    fn token_resolution() {
        ...
        let (addr, native) = rpc.resolve_token(eth, "0x1234567890abcdef1234567890abcdef12345678").unwrap();
        assert!(!native);
